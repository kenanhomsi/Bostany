    ...
    return (
        <div className="bg-BaserbodyLigh z-[1000] py-6 px-4 w-[264px] flex h-screen flex-col   gap-10  fixed ">
            ...
            <div className="flex flex-col justify-between py-2  h-full">
                <div className="flex flex-col gap-6">
                    {listData.map((ele, index) => (
                        <Link to={ele.link} key={index} className={`flex justify-between w-full py-3 px-4 rounded-2xl ${pathname == ele.link && 'bg-BaserSurfaceDarker'} `}>
                            <div className={`flex items-center gap-2 ${pathname == ele.link && `${DashBoard == 'Baser' ? ' text-BaserPrimary' : 'text-BostanyPrimary'}`}`}>
                ...
                {DashBoard == 'Baser' && <div onClick={handleBuyCoinesClick} className="flex    justify-between cursor-pointer items-center bg-BaserPrimaryContainer text-BaserFoshiy rounded-2xl p-2">
